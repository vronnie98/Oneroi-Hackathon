...
if has_train: train = static_feature_engineering(train)
if has_test:  test = static_feature_engineering(test)

# ==========================================
# 5. Dynamic Feature Engineering (Fold-Safe)
# ==========================================
print("Applying dynamic feature engineering...")


def compute_time_bin_edges(train_fold):
    pub_am_mask = (train_fold.get('mode', pd.Series(dtype=str)) == 'PUBLICTRANSPORT') & (train_fold['startime'] < 720)
    ...
    pub_pm_mask = (train_fold.get('mode', pd.Series(dtype=str)) == 'PUBLICTRANSPORT') & (train_fold['startime'] >= 720)
...
# Global application for base CSV exports (Should be used per-fold in CV)
if has_train:
    edges = compute_time_bin_edges(train)
    train = add_time_of_day(train, edges)
if has_test:
    if not has_train:
        print("Warning: Missing train reference, using test data for time bins.")
        edges = compute_time_bin_edges(test)
    test = add_time_of_day(test, edges)

# ==========================================
# 6. Data Prep & Encoding
# ==========================================
...
if has_train:
    X_train, y_train, groups, label_encoder = prepare_for_model(train, is_train=True)
    train.to_csv("processed_train_wide.csv", index=False)
